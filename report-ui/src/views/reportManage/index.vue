...
export default {
      ...
      crudOption: {
        ...
        rowButtons: [
          ...
          },
          {
            ...
            click: this.previewUniver
          },
          {
            label: "分析报表设计",
            permission: "bigScreenManage:design",
            isHide: (row) => {
              return row.reportType === 'report_analysis'
            },
            click: this.designAnalysis
          },
          {
            label: "分析报表预览",
            permission: "bigScreenManage:view",
        ...
        buttons: {
          ...
          rowButtonsWidth: 450 // row自定义按钮表格宽度
        },
        // 表格列
        columns: [
          {
  ...
  methods: {
    ...
    preview(val) {
      ...
      } else if (val.reportType === "report_analysis") {
        routePath = "/analysis/viewer";
      } else {
    ...
    designUniver(val) {
      let routeUrl = this.$router.resolve({
        path: "/excelreport/designeruniver",
        query: {
          reportCode: val.reportCode
        }
      });
